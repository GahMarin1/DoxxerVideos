/* ===== BOOT + DIGITAÇÃO COM BARRA ===== */
const output=document.getElementById("terminalOutput");
const bootScreen=document.getElementById("bootScreen");
const ageModal=document.getElementById("ageModal");

let cursorVisible=true;
function updateCursor(){output.innerHTML=output.innerHTML.replace(/\|/g,'')+(cursorVisible?'|':'');}
setInterval(()=>{cursorVisible=!cursorVisible;updateCursor();},500);
...
function typeCommand(command,index=0,callback){
    if(index<command.length){
        ...
        fadeCardsProgress(index,command.length);
    }else if(callback)callback();
}

function fadeCardsProgress(index,length){
    const cards=document.querySelectorAll(".card");
    const progress=Math.floor((index/length)*cards.length);
    for(let i=0;i<=progress;i++){cards[i].classList.add("show");}
}
...
function autoBoot(){
    ...
    typeCommand(command,0,()=>{
        ...
        .then(data=>{
            ...
            }else continueBootSequence("localização desconhecida");
...
const letters="01"; const fontSize=14;
...
/* KONAMI TOUCH */
let touchSequence=[]; const konamiTouch=["up","up","down","down","left","right","left","right","B","A"];
let startX=0,startY=0;
const btnB=document.getElementById("btnB"); const btnA=document.getElementById("btnA");
